      ...
      <div id="profileForm">
        ${Object.entries(profileData).map(([key, value]) => `
          <div class="profile-field" data-key="${key}">
            <label>${key === 'fullName' ? 'ФИО' : key === 'phone' ? 'Телефон' : 'Email'}:</label>
            <div class="profile-value">
              ${editingField === key
                ? `<input type="${key === 'email' ? 'email' : 'text'}" value="${value}" class="edit-input" placeholder="Введите данные">
                   <button class="save-btn">${svgSave}</button>`
                : `<span class="field-text">${value || 'Введите данные'}</span>
  ...
  function setupProfileEventListeners() {
    document.querySelectorAll('.edit-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        renderTab('profile');
        setTimeout(() => {
          const input = document.querySelector('.profile-field[data-key="' + key + '"] input');
    ...
    if (tab === 'appointments') {
      document.querySelectorAll('.cancel-btn').forEach(button => {
        button.addEventListener('click', () => {
          ...
          const confirmCancel = confirm("Вы действительно хотите отменить визит?");
          if (confirmCancel) {
